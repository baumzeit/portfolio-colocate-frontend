...
export const ProjectBanner = ({
  ...
  return (
    <div className={`grid group ${className} overflow-hidden drop-shadow-md`} style={{ gridTemplateAreas: '"banner"' }}>
      <div style={layerStyle}>
        <GatsbyImage
          ...
          className={`object-cover object-center w-full h-full transition-all duration-700 ${
            !hideOverlay ? 'group-hover:scale-[1.01] ease-out opacity-90 group-hover:opacity-100' : ''
          }`}
        />
      </div>
      ...
      {!hideTitle && (
        <div className={`flex ${isEven ? 'justify-start' : 'justify-end'} items-start `} style={{ gridArea: 'banner' }}>
          <div
            className={`z-10 py-0.5 mt-[8%] px-2.5 shadow-sm ${
